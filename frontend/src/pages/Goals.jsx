...
const COLORS = ['#059669', '#0D9488', '#2563EB', '#D97706', '#DC2626']
...
export default function Goals() {
  ...
  return (
    <div style={{ padding: 28, maxWidth: 800, position: 'relative' }}>
      <div className="orb" style={{ width: 400, height: 400, background: 'radial-gradient(circle,rgba(5,150,105,0.06),transparent)', top: -100, right: -50 }} />
      <div style={{ position: 'relative', zIndex: 1 }}>
        <div className="fade-up" style={{ marginBottom: 24 }}>
          <p style={{ fontSize: 11, color: 'var(--muted)', textTransform: 'uppercase', letterSpacing: '1px', marginBottom: 4, fontWeight: 600 }}>{data.length} active goals</p>
          <h1 style={{ fontSize: 32, fontWeight: 800, letterSpacing: '-1px' }}>Goals</h1>
        </div>
            ...
            return (
              <div key={g.id} className="card fade-up" style={{ padding: 18, animationDelay: `${i * 60}ms` }}>
                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: 12 }}>
                  <div>
                    <p style={{ fontSize: 14, fontWeight: 600, color: 'var(--text)', marginBottom: 3 }}>{g.emoji} {g.title}</p>
                ...
                <div style={{ display: 'flex', justifyContent: 'space-between', fontSize: 11, color: 'var(--muted)' }}>
                  ...
                  <span style={{ color: pct >= 100 ? '#059669' : pct >= 50 ? color : 'var(--muted)', fontWeight: 600 }}>
        ...
        {/* Afford calculator */}
        <div className="card fade-up" style={{ padding: 24, animationDelay: '200ms', border: '1px solid rgba(5,150,105,0.18)', background: 'rgba(5,150,105,0.03)' }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: 10, marginBottom: 6 }}>
            <div style={{ width: 8, height: 8, borderRadius: '50%', background: '#059669', animation: 'blink 2s ease-in-out infinite' }} />
            <p style={{ fontSize: 16, fontWeight: 700, color: 'var(--text)' }}>Can I afford this?</p>
          </div>
          ...
          <div style={{ display: 'flex', gap: 10, marginBottom: 12 }}>
            ...
            <button className="btn-primary" onClick={ask} disabled={loading}
              style={{ padding: '12px 20px', whiteSpace: 'nowrap', opacity: loading ? 0.7 : 1 }}>
              {loading ? '...' : 'Ask AI'}
            </button>
          </div>
          {result && (
            <div className="scale-in" style={{
              background: result.verdict === 'yes' ? 'rgba(5,150,105,0.06)' : 'rgba(220,38,38,0.06)',
              border: `1px solid ${result.verdict === 'yes' ? 'rgba(5,150,105,0.22)' : 'rgba(220,38,38,0.22)'}`,
              borderRadius: 12, padding: '14px 16px'
            }}>
              <p style={{ fontSize: 15, fontWeight: 700, color: result.verdict === 'yes' ? '#059669' : '#DC2626', marginBottom: 6 }}>{result.headline}</p>
