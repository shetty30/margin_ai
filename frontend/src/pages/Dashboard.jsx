...
const CATS = ['#059669', '#0D9488', '#2563EB', '#D97706', '#DC2626', '#7C3AED', '#64748B']
...
export default function Dashboard() {
  ...
  if (!data) return (
    <div style={{ padding: 32 }}>
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 220px', gap: 16 }}>
        {[1, 2, 3, 4].map(i => (
          <div key={i} style={{ height: 180, background: '#fff', borderRadius: 16, border: '1px solid var(--rim)', position: 'relative', overflow: 'hidden' }}>
  ...
  return (
    <div style={{ padding: 28, maxWidth: 960, position: 'relative' }}>
      {/* Background orbs */}
      <div className="orb" style={{ width: 500, height: 500, background: 'radial-gradient(circle,rgba(5,150,105,0.07),transparent)', top: -100, right: -100, zIndex: 0 }} />
      <div className="orb" style={{ width: 300, height: 300, background: 'radial-gradient(circle,rgba(13,148,136,0.05),transparent)', bottom: 100, left: -50, zIndex: 0 }} />
      ...
      <div style={{ position: 'relative', zIndex: 1 }}>
        ...
        <div className="fade-up" style={{ marginBottom: 24, display: 'flex', alignItems: 'flex-end', justifyContent: 'space-between' }}>
          <div>
            ...
            <h1 style={{ fontSize: 42, fontWeight: 800, letterSpacing: '-1.5px', color: 'var(--text)', lineHeight: 1 }}>
              <AnimatedNumber value={remaining} />
            </h1>
          </div>
          <div style={{ display: 'flex', gap: 8 }}>
            <div className="pill pill-violet">{pct}% used</div>
            <div className="pill" style={{ background: 'rgba(0,0,0,0.04)', color: 'var(--muted)' }}>{now.getDate()} days in</div>
          </div>
        </div>
        ...
        <div className="fade-up" style={{ animationDelay: '60ms', marginBottom: 24 }}>
          ...
          <div style={{ display: 'flex', justifyContent: 'space-between', marginTop: 8 }}>
            <span style={{ fontSize: 11, color: 'var(--faint)', fontWeight: 500 }}>{fmt(data.total_spent)} spent</span>
            <span style={{ fontSize: 11, color: 'var(--faint)', fontWeight: 500 }}>{fmt(data.expense_budget)} budget</span>
          </div>
        </div>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3,minmax(0,1fr))', gap: 12, marginBottom: 20 }}>
          {[
            { label: 'Income',    value: data.income,         color: '#059669', delay: '80ms' },
            { label: 'Saved',     value: data.savings_target, color: '#0D9488', delay: '120ms' },
            { label: 'Daily avg', value: data.daily_avg,      color: data.daily_avg > (data.expense_budget / 30) ? '#DC2626' : '#059669', delay: '160ms' },
          ].map(s => (
        ...
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 200px', gap: 12, marginBottom: 20 }}>
          ...
          <div className="card fade-up" style={{ padding: '18px 20px', animationDelay: '200ms' }}>
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 16 }}>
              <p style={{ fontSize: 11, color: 'var(--muted)', textTransform: 'uppercase', letterSpacing: '0.8px', fontWeight: 600 }}>Daily spend</p>
          ...
          <div className="card fade-up" style={{ padding: '18px 20px', animationDelay: '240ms' }}>
            <p style={{ fontSize: 11, color: 'var(--muted)', textTransform: 'uppercase', letterSpacing: '0.8px', marginBottom: 12, fontWeight: 600 }}>Categories</p>
        ...
        <div className="card fade-up" style={{ padding: '20px', animationDelay: '280ms' }}>
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 16 }}>
            <p style={{ fontSize: 11, color: 'var(--muted)', textTransform: 'uppercase', letterSpacing: '0.8px', fontWeight: 600 }}>Recent transactions</p>
            <a href="/transactions" style={{ fontSize: 12, color: '#059669', textDecoration: 'none', fontWeight: 600 }}>View all →</a>
          </div>
          {data.recent_txns.length > 0 ? data.recent_txns.map((t, i) => (
            <div key={i} className="slide-right" style={{
              display: 'flex', justifyContent: 'space-between', alignItems: 'center',
              padding: '11px 0', borderBottom: i < data.recent_txns.length - 1 ? '1px solid var(--rim)' : 'none',
              animationDelay: `${300 + i * 40}ms`
            }}>
              <div style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
                <div style={{ width: 36, height: 36, borderRadius: 11, background: 'rgba(5,150,105,0.08)', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 14, flexShrink: 0 }}>
                  💳
                </div>
                <div>
